...
def train(rank, args, shared_actor, shared_critic, train_data, valid_data, save_dir, 
    ...
    logger.info("Process %d: Begin training phase" % rank)
    ...
    for epoch in range(epoch_start_idx, dp.num_epoch):
        logger.info("Process %d: Start epoch %d" % (rank, epoch))
        ...
        for idx, data in enumerate(train_loader):
            ...
            if (idx + 1) % dp.log_size == 0:
                ...
                msg = '\tProcess %d: Batch %d/%d, mean_policy_losses: %2.3f, ' + \
                ...
                logger.info(msg % (rank, idx, len(train_loader), mm_policy_loss, 
        ...
        # Save the weights
        epoch_dir = os.path.join(save_dir, '%s-%s' % (rank, epoch))
...
def main(num_sensors=20, num_targets=10, config=None,
    ...
    if mode == 'train':
        ...
        for rank in range(0, num_processes):
            p = mp.Process(target=train, args=(rank, args, actor, critic, train_data, valid_data, save_dir, epoch_start, wp, dp))
            p.start()
            processes.append(p)
        ...
        #train(rank, args, actor, critic, train_data, valid_data, save_dir, epoch_start, wp, dp)
    else:
