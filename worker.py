...
def train(rank, counter, log_dir, lock, seed, 
            num_sensors, num_targets, shared_actor, 
            shared_critic, save_dir, config=None):
    ...
    actor_optim = optim.Adam(shared_actor.parameters(), dp.actor_lr)
    critic_optim = optim.Adam(shared_critic.parameters(), dp.critic_lr)
    ...
    while True:
        ...
        if (batch + 1) % dp.log_size == 0:
            ...
            global_step = int(batch / dp.log_size)
            writer.add_scalar('process ' + str(rank) + ' - batch/policy_loss', mm_policy_loss, global_step)
            writer.add_scalar('process ' + str(rank) + ' - batch/entropy', mm_entropies, global_step)
            writer.add_scalar('process ' + str(rank) + ' - batch/net_lifetime', m_net_lifetime, global_step)
            writer.add_scalar('process ' + str(rank) + ' - batch/mc_travel_dist', m_mc_travel_dist, global_step)
            writer.add_scalar('process ' + str(rank) + ' - batch/reward_per_step', mm_rewards, global_step)
            writer.add_scalar('process ' + str(rank) + ' - batch/num_steps', m_steps, global_step)
            writer.add_scalar('process ' + str(rank) + ' - batch/total_rewards', m_total_rewards, global_step)
            writer.add_histogram_raw('process ' + str(rank) + ' - batch/action_hist', 
                                        min=values.min(),
                                        max=values.max(),
                                        num=len(values),
                                        sum=values.sum(),
            ...
            msg = '\tProcess %d - batch %d: mean_policy_losses: %2.3f, ' + \
        ...
        if (batch + 1) % dp.train_size == 0:
            ...
            # Save the weights
            epoch = int((batch + 1) / dp.train_size)
            epoch_dir = os.path.join(save_dir, 'process %s - epoch %s' % (rank, epoch))
            ...
            writer.add_scalar('process ' + str(rank) + ' - epoch/policy_loss', mm_policy_loss, epoch)
            writer.add_scalar('process ' + str(rank) + ' - epoch/entropy', e, epoch)
            writer.add_scalars('process ' + str(rank) + ' - epoch/net_lifetime', {
                                'train': m_net_lifetime,
                                'valid': m_net_lifetime_valid 
                            }, epoch)
            writer.add_scalars('process ' + str(rank) + ' - epoch/mc_travel_dist', {
